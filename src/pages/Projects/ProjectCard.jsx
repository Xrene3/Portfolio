...
export default function ProjectCard({ project, children, openModal, openProjectModal, setIsHoveringCard, play, stop }) {
    return (
        < >
            <Card title={project.name && project.name} className="audio-on-hover w-full md:max-w-md md:min-w-120  md:hover:rotate-1 ease-linear duration-100"
                onMouseEnter={() => {
            ...
            >
                ...
                <div className='absolute bottom-0 right-0 p-5'>
                    <button className="transition ease-linear duration-300  dark:hover:text-lime-200 py-2 px-5 hover:bg-neutral-600/20 rounded-full" onClick={() => openProjectModal(project)}>ShowMore</button>
                </div>
            </Card >
        </>
    );
